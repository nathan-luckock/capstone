...
- **Exhaustive model checking.** Where the crash sims sample random interleavings, from-scratch bounded model checkers enumerate *every* reachable interleaving of an abstract model and prove an invariant holds in all of them, each with a deliberately buggy variant confirming the check has teeth. Four are proved this way: the write-ahead-logging ordering invariant (`walmodel`, a page change is never durable ahead of its log record), the MVCC snapshot read-stability invariant (the `txn` model), and, through the approximate index, both tenant isolation and cache freshness (`rlsmodel`, below). Random testing finds bugs; this proves their absence over the bounded model.
