...
impl Cluster {
    ...
    /// Run anti-entropy to a fixed point: every pair of nodes that can currently
    /// reach each other exchanges and merges full state until a whole pass
    /// changes nothing. Within each connected partition group, the nodes
    /// converge. (Full-state merge here; Merkle-diff repair is a later phase.)
    pub fn anti_entropy(&mut self) {
        loop {
            let snapshot = self.nodes.clone();
            for a in 0..self.nodes.len() {
                for (b, peer) in snapshot.iter().enumerate() {
                    if a != b && self.reachable(a, b) {
                        self.nodes[a].merge(peer);
                    }
                }
            }
            let changed =
                (0..self.nodes.len()).any(|i| self.nodes[i].slots() != snapshot[i].slots());
            if !changed {
                break;
            }
        }
...
pub fn run_seed(seed: u64, nodes: usize, ops: usize) -> SimReport {
    ...
    let keyspace = 24u64;
    let (mut writes, mut partitions) = (0usize, 0usize);
...
mod tests {
    ...
    #[test]
    fn a_thousand_seeds_all_converge() {
        for seed in 0..1000u64 {
            let report = run_seed(seed, 5, 300);
