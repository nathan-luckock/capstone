//! Differential testing of the rustdb engine against SQLite.
//!
//! For each seed, a generator builds a small random schema, some rows, and a
//! random `SELECT`, then runs the identical SQL through both rustdb and SQLite
//! (the reference oracle) and compares the results. Any divergence is a rustdb
...
//!
//! SQLite and rustdb are not the same dialect (SQLite is dynamically typed,
//! rustdb is statically typed like Postgres). To compare meaningfully, the
...
//! the SQL, the seed is skipped rather than blamed on rustdb.
...
pub enum Outcome {
    /// rustdb and SQLite agreed; the query returned `rows` rows.
    Match {
        /// Rows returned.
        rows: usize,
    },
...
fn gen_aggregate(rng: &mut Rng) -> String {
    ...
    // SQLite would coerce a TEXT aggregate compared to an int; rustdb, like
...
/// # Errors
///
/// Returns a human-readable report (including the seed and full SQL) if rustdb
...
pub fn run_seed(seed: u64) -> Result<Outcome, String> {
    ...
    // SQLite is the oracle; if it rejects the SQL, skip rather than blame rustdb.
    let Some(expected) = run_sqlite(&prog) else {
        return Ok(Outcome::Skipped);
    };

    let actual = run_rustdb(&prog).map_err(|e| {
        report(
            seed,
            &prog,
            &format!("rustdb rejected valid SQL: {e}"),
            None,
            None,
        )
    })?;
...
/// Run the program through rustdb, returning the query's rows as a sorted
/// multiset of canonical cells.
fn run_rustdb(prog: &Program) -> Result<Vec<String>, String> {
    ...
    match db.execute(&prog.query).map_err(|e| e.to_string())? {
        QueryOutcome::Rows { rows, .. } => {
            let mut out: Vec<String> = rows
                .iter()
                .map(|row| row.iter().map(canon_rustdb).collect::<Vec<_>>().join("|"))
                .collect();
            out.sort();
            Ok(out)
        }
...
/// Canonical text for a rustdb value (matches [`canon_sqlite`]).
...
/// Canonical text for a SQLite value (matches [`canon_rustdb`]).
...
mod tests {
    ...
    #[test]
    fn sqlite_and_rustdb_agree_on_a_handful_of_seeds() {
        for seed in 0..32u64 {
